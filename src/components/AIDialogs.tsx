...
import { Send, Sparkles, AlertCircle, RefreshCw, BookOpen, User, Bot, HelpCircle } from "lucide-react";
...
interface AIDialogsProps {
  initialMode?: "tutor" | "research" | "coach";
}

export default function AIDialogs({ initialMode = "tutor" }: AIDialogsProps) {
  const [mode, setMode] = useState<"tutor" | "research" | "coach">(initialMode);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-[#E2E8F0] overflow-hidden flex flex-col h-[650px]" id="ai-chat-portal">
      {/* Header Tabs */}
      <div className="bg-white border-b border-[#E2E8F0] p-4">
        ...
        <div className="grid grid-cols-3 gap-2 bg-[#F1F5F9] p-1.5 rounded-xl border border-[#E2E8F0]">
          <button
